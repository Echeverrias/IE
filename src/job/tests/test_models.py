...
class TestJob(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.cities = []
        for i in range(0,2):
            clscities.append(City.objects.create(name=str(i)))
    ...
    def test_add_cities(self):
        job = Job.objects.get(id=555)
        for city in TestJob.cities:
            job.cities.add(city)
        self.assertEqual(Job.objects.filter(id=555).count(), 1)
        self.assertEqual(Job.objects.get(id=555).cities.all().count(), len(TestJob.cities))
    ...
    def test_add_cities_method(self):
        ...
        self.assertEqual(Job.objects.get(id=777).cities.all().count(), len(TestJob.cities))
